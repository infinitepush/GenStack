...
import { extractPromptIntent } from "./prompt-intent.js";
...
export class PipelineEngine {
  ...
  async run(input: { prompt: string; seedConfig?: unknown }): Promise<PipelineRunResult> {
    ...
    const generationMode = draft.structured === undefined ? "fallback" : "structured";

    const parsed = await timedStage(
      stages,
      "parse",
      async () => draft.structured === undefined ? parsePossiblyBrokenJson(draft.text) : {
        value: draft.structured,
        repaired: false,
        source: draft.text
      },
      () => draft.structured === undefined ? "Draft JSON parsed." : "Structured AppConfig received from provider.",
    ...
    let finalEvaluation = await timedStage(
      stages,
      "evaluate",
      async () => evaluateConfig(repair.config, prompt),
    ...
    if (mismatchedDomain) {
      ...
      finalEvaluation = await timedStage(
        stages,
        "fallback evaluate",
        async () => evaluateConfig(finalRepair.config, prompt),
    ...
    return {
      ...
      model: draft.model,
      generationMode,
      repairActions: finalRepair.changes.length,
      intent,
      stages,
      rawDraft: draft.text,

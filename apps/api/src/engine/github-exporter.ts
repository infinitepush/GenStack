...
async function githubFetch<T>(url: string, token: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    if (response.status === 401) throw new Error("Invalid GitHub token (Unauthorized)");
    if (response.status === 403) {
      const remaining = response.headers.get("x-ratelimit-remaining");
      throw new Error(remaining === "0" ? "GitHub rate limit exceeded, try again in 60s" : "Access forbidden: Token missing 'repo' scope or has insufficient permissions");
    }
    const body = (await response.json().catch(() => null)) as { message?: string; errors?: Array<{ resource: string; code: string; field: string; message?: string }> } | null;
    let detail = "";
    if (body?.errors && body.errors.length > 0) {
      detail = ": " + body.errors.map(e => e.message ?? `${e.field} ${e.code}`).join(", ");
    }
    throw new Error(body?.message ? `${body.message}${detail}` : `GitHub API returned status ${response.status}${detail}`);
  }
